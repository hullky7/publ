...
def index():
    ...
    # оновлення ціни (price) та визначення вартості акцій (total)
    total_full = 0  # сума вартості акцій та кешу на рахунку
    ...
    # форматуємо price та total під долари
...
def sell():
    ...
    if request.method == "POST":
        ...
        stack = lookup(request.form.get("symbol"))
        if not stack:
            return apology("Must provide true stock’s symbol", 400)
        symbol = stack["symbol"]
        ...
        stocks1 = db.execute("SELECT symbol FROM portfolio WHERE id=:id", id=session["user_id"])
        return render_template("sell.html", stocks=stocks1)
